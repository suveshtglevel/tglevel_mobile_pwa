...
export default function Notifications() {
  ...
  return (
    <main className="w-full h-[100dvh] bg-[#e4e9f0] flex justify-center overflow-hidden">
      <div className="flex flex-col h-full w-full max-w-md bg-[#e4e9f0] shadow-xl relative overflow-hidden">

        {/* Top Bar */}
        <div className="flex-none flex items-center bg-[#e4e9f0] px-4 py-3 relative">
          <button
            onClick={() => router.back()}
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-6 flex flex-col gap-3">
          {list.length === 0 ? (
            ...
            list.map((n) => (
              <button
                key={n.id}
                className="w-full bg-white rounded-2xl px-4 py-4 shadow-sm flex items-center gap-3 active:scale-[0.99] transition-transform text-left"
              >
                {/* Bell Icon */}
